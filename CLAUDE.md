...
**xhs-so 搜索命令**：`feedgrab xhs-so <keyword>` 纯 API 搜索 + 互动排序汇总表（MD + CSV），仿 `x-so` 模式。支持 `--sort popular/latest`、`--type video/image`、`--save` 保存单篇。
...
| v0.9.12 | `feedgrab doctor` 诊断命令（按平台分区：x/xhs/mpweixin，一键检查 Cookie/依赖/网络） |
