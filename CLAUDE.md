...
| B站 | API |
| 微信公众号 | Jina → Playwright WeChat JS 提取（单篇 + markdownify 富文本）/ 搜狗搜索 → Playwright 跳转 → 全文抓取（关键词搜索） |
| Telegram | Telethon |
| RSS | feedparser |
| 任意网页 | Jina 兜底 |
...
Tier 1 Jina → Tier 2 Playwright WeChat JS 提取。Tier 2 使用 `WECHAT_ARTICLE_JS_EVALUATE` 在 JS 层面一次性提取 9 类数据（DOM 元素 + OG meta + JS 脚本变量 + `#js_content` HTML）。`create_time` 通过三层正则从页面 JS 脚本提取（JsDecode 格式 / 单引号 / 双引号），精确到秒。`msg_cdn_url` 比 `og:image` 质量更高。`_html_to_markdown()` 使用 markdownify + BeautifulSoup 预处理（lazy image 转换、SVG 过滤、WeChat 代码块占位符策略）。输出的 Markdown 在 front matter 后插入 `<meta name="referrer" content="no-referrer">`，避免 mmbiz.qpic.cn 图片 Referer 校验 403。
