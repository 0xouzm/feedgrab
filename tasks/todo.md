...
> 来源：`feedgrab-desktop` 提交 `fa40579b`，本次 cherry-pick 回 main（版本 v0.26.2）。
> 用户报告：用 v0.1.22 桌面客户端抓书签文件夹「银行卡」，产出目录里有三个标题相同、带 hash 后缀的
> md，看起来像重复文件。要求「检查下什么原因」。

---

## ✅ 诊断（已完成）

- [x] 全目录内容级重复检测：44 文件 / 唯一正文 42 / 重复组 2
- [x] 分辨两类：@QuanIsMine 那组是作者自己隔月重发的两条真实推文（**不是 bug**）；老V 那组才是缺陷
- [x] 直接调 `fetch_tweet_by_rest_id` 查三条推文的真实身份 —— **推翻了自己的第一版推断**：
      原以为是「转推被存成转推者 URL」，实测三条 `是否转推: 否`，真实情况是别人在 thread 下的**回复**
- [x] 定位根因链：`_classify_tweet` 把任何回复送进线程路径 → `fetch_tweet_thread` 重建的是根推作者
      自回复链 → `_is_same_thread` 首条判据 `user_id != root_user_id` 把目标过滤掉 → 组装只用
      `thread_tweets[0]`，传入的 `tweet_id` 被丢弃
- [x] 实证根因：传入 @ClockWorkMe 的回复 id，返回 `author=@LaoVStories`、目标推文**不在结果里**
- [x] 确认影响面：不止书签，`_fetch_via_graphql` 无条件先跑 thread，全部单条推文抓取同样中招

## ✅ 实施（已完成，只改一处）

- [x] `twitter_thread.py`：同作者过滤后、组装前加闸——目标推文不在 `thread_tweets` 里就返回 `None`
- [x] 不改 `_classify_tweet`（线程路径本就该处理回复，问题在于线程路径自己要能识别「目标不属于本链」）
- [x] 不改书签侧：其 thread 分支调的是 `_fetch_via_graphql`，fallback 在其内部，自动受益

## ✅ 测试（已完成）

- [x] 新增 `tests/test_twitter_thread_target.py` 6 例
- [x] **验证测试本身有效**：`git stash` 撤销修复后目标用例如期失败并返回了根推作者的线程数据，
      恢复后 6 例全过 —— 排除「测试写了但抓不到 bug」
- [x] main 线全量 310 → **316 passed**；service_layer + rendering + dispatch 交叉回归 17 passed

## ✅ 真实重抓验证（已完成）

- [x] 备份 3 个错误 md + 去重索引到 `.tmp/broken-md-backup/`
- [x] 删除 3 个 md，并从索引移除对应 item_id（12071 → 12068，否则重抓必被跳过）
- [x] 重跑 `https://x.com/i/history/bookmarks/2019055008676040850`，日志打出新逻辑，
      成功 3 / 跳过 50 / 失败 1（TweetTombstone，账号被封，正常）
- [x] 三份新产物 source / author / 正文一一对应
- [x] 全目录复检：重复组 2 → **1**（仅剩非 bug 的那组），撞名 hash 后缀文件 2 → **0**

## 复盘

**亮点**：
- 没有停在用户描述的表象上。「重复文件」听起来像去重索引失效，实际 `source` 和 `item_id` 全是对的，
  真正的缺陷是**内容被替换、被收藏那条的正文丢失**——比重复严重得多，且影响面覆盖全部单条推文抓取
- 修复只动一行判断：先读懂 `twitter.py` 已有的 fallback 正好是想要的行为，就不必新写退化逻辑

**做错的、值得记住的**（已沉淀进 `tasks/lessons.md`）：
- 第一版根因推断（转推被存成转推者 URL）是错的，靠实测纠正。诊断阶段的推断必须当假设看待，
  下结论前先查目标对象的真实身份
- 「用户说的现象」和「真正的缺陷」可能不是一回事，修之前先确认症状与根因的因果链完整
