...
3 次 API 调用完成抓取：`/repos/{owner}/{repo}`（元数据）→ `/repos/{owner}/{repo}/contents/`（根目录列表）→ README 内容。中文 README 优先：从根目录列表匹配 8 种变体（`README_CN.md`、`README.zh-CN.md` 等），命中则获取中文版，否则用默认 README。**子目录中文 README 搜索**：根目录未命中时，`_find_chinese_readme_from_content()` 解析默认 README 中的语言导航链接（如 `[**中文**](docs/README.zh-CN.md)`），6 种中文标记模式 + bold/italic 兼容，支持相对路径和完整 GitHub URL，零额外 API 调用。**相对图片链接补全**：`_resolve_relative_urls()` 将 Markdown `![](path)` 和 HTML `<img src="path">` 的相对路径转为 `raw.githubusercontent.com` 绝对 URL，基于 README 所在目录计算 base_dir，`../` 路径规范化。`_extract_readme_summary()` 从 README 提取第一行有意义的描述文本作为标题（跳过 heading、badge、HTML、blockquote、短于 15 字符的行）。`parse_github_url()` 统一处理仓库首页/blob/tree/issues 等 URL，取前两段 path 为 owner/repo。`item_id = MD5("{owner}/{repo}")[:12]` 保证仓库级别去重。无 Token 60 次/小时，有 Token 5000 次/小时。
