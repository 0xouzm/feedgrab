...
| 飞书/Lark | Open API → CDP 直连 → Playwright PageMain Block 树 → Jina（单篇 + 知识库批量 + 嵌入表格 + 图片下载） |
...
| Telegram | Telethon |
| RSS | feedparser |
| 付费新闻网站（300+） | 7 级 Tier 付费墙绕过（JSON-LD → Googlebot/Bingbot UA → Referer 伪装 → AMP → EU IP → archive.today → Google Cache → Jina 兜底） |
...
│   │   ├── feishu.py          # 飞书单篇（Open API → CDP 直连 → Playwright PageMain → Jina + Block→MD + Sheet Protobuf 解码 + 图片下载）
...
│   │   └── paywall.py         # 付费墙 7 级 Tier 级联绕过（融合自 qiaomu fetch_url.sh：Googlebot/Bingbot UA + AMP + archive.today + Google Cache + JSON-LD 前置，300+ 付费站点）
│   └── utils/
│       ├── storage.py         # 按平台分目录 Markdown 输出 + YAML front matter
│       ├── dedup.py           # 全局去重索引
│       ├── http_client.py     # 统一 HTTP 客户端（curl_cffi TLS 指纹 → requests fallback）
│       ├── jsonld.py          # Schema.org JSON-LD articleBody 提取（NewsArticle/Article/BlogPosting 等 10 种类型，@graph 嵌套，多 author 形态）
...
**嵌入电子表格**：Canvas 渲染无 DOM，通过 `POST /space/api/v3/sheet/client_vars` 内部 API 获取 JSON（Protobuf gzip+base64 编码的单元格数据），5 层 wire format 解码（`L0[1]→L1[2]→L2[12]→L3[2]→L4[repeated 2]`）提取 cell 字符串，reshape 为 2D 数组渲染 GFM 表格。**懒加载修复**：真实飞书 Sheet 会在滚动到可视区后额外请求 `/space/api/v3/sheet/block`，因此 Playwright/CDP 路径会先做分段滚动预热，再把 `sheet/block` 返回的 blocks 合并回 `client_vars.snapshot.blocks`。**错位修复**：slot blob 优先解析 field 1 的 packed varint 序列，而不是只依赖 field 2 descriptor，这样可以正确恢复被重复复用的厂商/类型/价格单元格，避免第二行开始整体错列。`browser.py` 中 `FEISHU_SHEET_FETCH_JS` 和 `_capture_sheet_response()` 拦截器提供多策略数据获取。
...
### 付费墙绕过 + JSON-LD 提取（`fetchers/paywall.py` + `utils/jsonld.py`）

**融合自** qiaomu-anything-to-notebooklm 的 `scripts/fetch_url.sh`（见 `迭代方案调研报告/20260419-qiaomu-anything-to-notebooklm对比分析.md`）。`reader.py` 的 `_fetch()` 兜底分支改造为 `try_paywall_bypass() → Jina` 二级兜底。

**Tier 0-7 级联**：`_try_direct_jsonld()`（非付费站点也跑，<1s 直接拿 articleBody）→ `_try_tier1_googlebot()`（22 个 GOOGLEBOT_DOMAINS）→ `_try_tier2_bingbot()`（4 站）→ `_try_tier3_generic()`（a/b/c/d 四策略 Googlebot/Bingbot/FB/Twitter Referer，对 44 个 PAYWALL_DOMAINS）→ `_try_tier4_amp()`（AMP 4 种 URL 模式 + `.html→.amp.html` 重写，12 站）→ `_try_tier5_eu_ip()`（`185.X.X.X` 随机 EU IP）→ `_try_tier6_archive_today()`（CAPTCHA 自动检测 → `logger.warning` 跳过）→ `_try_tier7_google_cache()`。全部失败返回 `None` → Jina 接手。

**HTML 双解析通道**：`_extract_article_from_html()` 先调 `extract_jsonld_article()` 拿 Schema.org NewsArticle/Article/BlogPosting 等 10 种类型的 `articleBody`（最长优先 + `@graph` 嵌套 + 多 `@type`/author 形态）；失败回退 `_html_to_markdown()`（markdownify + BeautifulSoup 剥离 script/style/nav/footer/aside/form）。策略标记形如 `tier3a.googlebot+jsonld` / `tier4.amp/amp+markdown`，写入 `UnifiedContent.extra.strategy`。

**内容质量闸**：`_has_content()`（500+ 字符 + 8+ 行 + 过滤 404/Access Denied/"Don't miss what's happening"）、`_is_paywall_content()`（20+ 种付费墙文案正则）、`_is_captcha_page()`（Cloudflare Challenge/recaptcha/hcaptcha）。

**HTTP 层**：复用 `utils/http_client.py` 的 curl_cffi Chrome TLS 指纹，但每次覆盖 `User-Agent`（Googlebot `Mozilla/5.0 (compatible; Googlebot/2.1; ...)`、Bingbot、Chrome macOS）+ Referer + `X-Forwarded-For` + `cookies={}` 清空（等价于 `curl -b ""`）。

**新增 `SourceType.WEB`**：付费墙抓到的内容落到 `output/Web/` 独立目录，`author`/`published`/`image`/`strategy` 存 `extra`。`from_web(data)` 工厂函数。

**配置项 7 个**：`PAYWALL_ENABLED`（默认 true 总开关）、`PAYWALL_TIMEOUT`（15s）、`PAYWALL_USE_AMP` / `PAYWALL_USE_ARCHIVE` / `PAYWALL_USE_GOOGLE_CACHE`（均 true）、`PAYWALL_DOMAINS_EXTRA`（用户追加域名管道分隔）、`PAYWALL_JSONLD_FOR_ALL`（默认 true，让 Tier 0 对所有 generic URL 都跑，显著加速 SEO 友好站点）。

**4 套域名列表**（硬编码 + env 追加）：`GOOGLEBOT_DOMAINS`（22 站 WSJ/FT/Economist/Telegraph/Times/Le Monde/Spiegel 等）、`BINGBOT_DOMAINS`（4 站 Haaretz/NZ Herald/Stratfor/TheMarker）、`FACEBOOK_REF_DOMAINS`（4 站）、`AMP_DOMAINS`（12 站）、`PAYWALL_DOMAINS`（44 站合集）。域名匹配 `_match_domain()` 兼容 `www.` 前缀和子域名。

**覆盖单元测试**：`tests/test_paywall.py` 26 条（JSON-LD 单对象/`@graph`/多 `@type`/多 author/多块取最长/malformed → None；`_has_content`/`_is_paywall_content`/`_is_captcha_page` 边界；`_match_domain` www/子域/空列表；`is_paywall_domain` 三表联合；域名列表格式合法性正则）。

**技术来源** [Bypass Paywalls Clean](https://gitflic.ru/project/magnolia1234/bypass-paywalls-chrome-clean) — 搜索引擎白名单 UA 伪装 + JSON-LD articleBody 提取是核心原理。
...
| 版本 | 功能 |
|------|------|
| v0.16.0 | 付费墙绕过 + JSON-LD articleBody 提取（融合 qiaomu `fetch_url.sh`）：7 级 Tier 级联（Googlebot UA + AMP + archive.today + Google Cache）+ 300+ 付费站点支持 + `SourceType.WEB` 独立目录 + 26 条单元测试 |
| v0.15.2 | 飞书嵌入表格错位修复（Sheet 懒加载分段预热 + `/sheet/block` 合并回 `client_vars` + packed slot 映射解码 + `tests/test_feishu_sheet_decode.py` 真实样本回归） |
...
| v0.12.5 | Claude Code Skill 发布：5 个标准 SKILL.md 技能（feedgrab/feedgrab-batch/feedgrab-setup/analyzer/video），支持 `npx skills add iBigQiang/feedgrab` 一键安装 |
...
| 付费墙 / 通用网页 | `paywall.py`（7 级 Tier 级联 + 300+ 域名库 + Googlebot/Bingbot UA）+ `utils/jsonld.py`（Schema.org articleBody 提取） |
