...
| LinuxDo / Discourse | 主路径不是 DOM 抓取，而是 `topic.json`：Tier 0 游客/已保存 session Cookie → Tier 1 CDP 页面内 `fetch(...json)` → Tier 2 Stealth Playwright 页面内 `fetch(...json)` → Tier 3 Jina；明确 404 / 私有 / 需登录时提前终止，不再把错误页写入 Markdown；折叠块采用简单 callout + 复杂 `<details class="feedgrab-fold">` 混合策略 |
