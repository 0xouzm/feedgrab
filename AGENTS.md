...
| LinuxDo / Discourse | Discourse Topic JSON API → CDP 复用 Chrome → Playwright 页面内 fetch → Jina（最后兜底） |
...
| LinuxDo / Discourse | `linuxdo.py`（topic JSON → CDP → 浏览器 → Jina） |
