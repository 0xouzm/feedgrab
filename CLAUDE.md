...
`X_DOWNLOAD_MEDIA=true` / `XHS_DOWNLOAD_MEDIA=true` 开启后，图片视频下载到 `{md_dir}/attachments/{item_id}/` 子目录，Markdown 中远程 URL 替换为相对路径。仿飞书 `download_feishu_images` 模式：`save_to_markdown()` 返回路径 → `download_media()` 下载 + 替换。URL 优化：Twitter `name=orig` 原图质量；XHS 去 `!nd_*` CDN resize 后缀 + `Referer: https://www.xiaohongshu.com` 防盗链。覆盖范围：单篇（`reader.py`）+ 全部 8 个批量 fetcher（bookmarks/user_tweets/list_tweets/search_tweets/keyword_search/api_user_tweets/xhs_user_notes/xhs_search_notes）。单张失败保留远程 URL，不影响其他下载。
