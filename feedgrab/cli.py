...
用法：
    ...
    feedgrab reddit-so <keyword> 按关键词搜索 Reddit 帖子
    feedgrab reddit-sub <name>   批量抓取 Reddit 子版块帖子
...
支持平台：
    ...
    Bilibili、小红书、知乎、Reddit、LinuxDo/IDCFlare、
    GitHub、飞书、金山、FlowUs、有道云、RSS，以及任意网页
...
示例：
    ...
    feedgrab x-so WorkBuddy --lang zh+zxx --sort all
    feedgrab reddit-so codex --sort comments --time all --limit 10
    feedgrab reddit-sub ChatGPT --sort hot --limit 25
...
""")
    ...
    if cmd == "setup":
        ...
        if len(sys.argv) < 3:
            ...
            print("   支持平台：twitter、xhs、wechat、feishu、kdocs、linuxdo、idcflare、flowus、reddit、zhihu、zsxq")
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            ...
            print('            feedgrab x-so WorkBuddy --lang zh+zxx --sort all')
            ...
            print("     --lang LANG        语言过滤：zh / zh+zxx / en / ja / 空值（默认：zh）")
            print("     --min-faves N      最低点赞数（默认：0）")
            print("     --min-retweets N   最低转推数（默认：0）")
            print("     --sort MODE        live=最新，top=热门，all=Live+Top 合并（默认：live）")
