...
## 2026-08-31 · v0.26.2 · 线程抓取不再把「别人的回复」替换成根推内容

> 从 `feedgrab-desktop` cherry-pick 回 main（源提交 `fa40579b`）。
> `twitter_thread.py` 两分支基线逐字节一致，源码为同一份，零冲突。
...
- 新增 `tests/test_twitter_thread_target.py` 6 例。**并验证了测试本身有效**：`git stash` 撤销修复后
  ...
  恢复修复后 6 例全过——排除「测试空转」。
- main 线全量 310 → **316 passed**；`test_service_layer` + `test_twitter_rendering` + `test_twitter_dispatch` 交叉回归 17 passed（desktop 分支另有 worker/service_desktop 两个文件，main 线不含，故此处组合与 desktop 条目不同）。
...
## 2026-08-31 · v0.26.1 · X 全渠道权限实测 + 账号私有渠道锁主账号

> 本条从 `feedgrab-desktop` 分支 cherry-pick 回 main（源提交 `e66f6465`）。
> 5 个 twitter fetcher 在两分支基线逐字节一致，源码为同一份；`cli.py` / `reader.py` 三方合并零冲突。
> **main 此前完全没有主账号锁定基础设施**（`load_primary_twitter_cookies` / `primary_only` 均 0 匹配），
> 连上一批「书签锁主账号」也不在 main 线上 —— 即用 main 的 CLI 抓书签会拿备用号轮换去读私有数据，
> 白跑一趟还掩盖真实原因。本次一并补齐。
...
方案文档：`docs/开发及迭代方案调研报告/2026-08-31_X平台全渠道权限实测与主账号锁定方案.md`（本地，目录已 gitignore）
...
### 主账号语义基础设施（main 线首次引入）

- `twitter_cookies.py`：主账号语义三件套 —— `_PRIMARY_SOURCE_LABELS` 白名单、
...
实测在 `feedgrab-desktop` 分支完成，跑的是与本条同一份 fetcher 源码（基线逐字节一致，见开头说明）：
...
main 线单元测试 264 → **310 passed**（+46，三个文件均为本轮新建）：
...
推送前按 `/ship` 跑完三关，又改出 6 处。都是"第一版能跑但边界不对"的类型（第 6 项属桌面端 GUI，不在 main 线）：
...
| 6 | 桌面端输入框示例仍写 legacy `https://x.com/i/bookmarks/<id>`（**GUI 侧，留在 `feedgrab-desktop` 分支**） | `desktop/renderer/src/App.tsx` 改为 `/i/history/bookmarks/<id>`，并补一行"X 总书签批量：`https://x.com/i/history`" |
