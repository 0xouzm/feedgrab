...
替代浏览器搜索补充的服务器友好方案（$0.15/千条）。配置 `TWITTERAPI_IO_KEY` 后超过 800 条时自动走 API 替代浏览器搜索。`X_API_PROVIDER=api` 可全量走付费 API（无需 Cookie）。使用 max_id 分页（Snowflake ID 递增），断点续传（JSONL 缓存），智能直保（普通推文直接保存，长文/线程走 GraphQL）。
